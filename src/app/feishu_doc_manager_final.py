...
class FeishuDocManagerWorking:
    ...
    def move_doc_to_wiki(self, space_id: str, parent_wiki_token: Optional[str], obj_type: str, obj_token: str, apply: bool = False) -> Dict[str, Any]:
        """将云空间文档移动到知识库
        参数:
            space_id: 目标知识库ID
            parent_wiki_token: 目标父节点token
            obj_type: 文档类型，如"doc"、"sheet"、"mindnote"等
            obj_token: 待迁移文档token
            apply: 若权限不足，是否提交迁入申请
        返回:
            接口返回的完整JSON数据。若返回task_id，需配合查询任务结果接口使用。
        """
        if not self.tenant_access_token:
            self.get_tenant_access_token()
        url = f"https://open.feishu.cn/open-apis/wiki/v2/spaces/{space_id}/nodes/move_docs_to_wiki"
        if apply:
            # 某些场景下需要以查询参数形式传递apply，触发迁入审批
            url += "?apply=true"
        headers = {
        ...
        # 不传 parent_wiki_token 时表示迁移到知识库根节点
        if parent_wiki_token:
            data["parent_wiki_token"] = parent_wiki_token
        response = requests.post(url, headers=headers, json=data)
        try:
            result = response.json()
        except Exception:
            raise Exception(f"移动文档到知识库失败，非JSON响应，状态码: {response.status_code}, 文本: {response.text[:200]}")
        if response.status_code != 200:
            raise Exception(f"移动文档到知识库失败，状态码: {response.status_code}, 返回: {result}")
        if result.get("code") != 0:
            raise Exception(f"移动文档到知识库失败: {result}")
        print("✅ 已调用移动文档至知识库接口")
        return result

    def get_wiki_task_result(self, task_id: str, task_type: str = "move") -> Dict[str, Any]:
        """查询知识库异步任务结果
        注意：具体URL以官方文档为准；此处按常见风格实现占位。
        返回原始JSON，包含状态与结果。
        """
        if not self.tenant_access_token:
            self.get_tenant_access_token()
        # 根据公开资料，此任务查询接口通常位于wiki任务查询端点。若官方有明确路径，请替换为权威URL。
        url = f"https://open.feishu.cn/open-apis/wiki/v2/tasks/{task_id}?task_type={task_type}"
        headers = {
            "Authorization": f"Bearer {self.tenant_access_token}",
            "Content-Type": "application/json; charset=utf-8"
        }
        ...
        try:
            ...
            raise Exception(f"查询任务失败，非JSON响应，状态码: {response.status_code}, 文本: {response.text[:200]}")
        if response.status_code != 200:
            raise Exception(f"查询任务失败，状态码: {response.status_code}, 返回: {result}")
        if result.get("code") != 0:
            raise Exception(f"查询任务失败: {result}")
        return result

    def move_doc_to_wiki_and_wait(self, space_id: str, parent_wiki_token: Optional[str], obj_type: str, obj_token: str, apply: bool = False, timeout_seconds: int = 120, poll_interval_seconds: float = 2.0) -> Dict[str, Any]:
        """移动文档到知识库并等待异步任务完成
        若接口直接成功返回，无task_id则立即返回；若返回task_id则轮询查询直至成功/失败/超时。
        返回包含最终结果或任务查询结果的JSON。
        """
        result = self.move_doc_to_wiki(space_id, parent_wiki_token, obj_type, obj_token, apply)
        data = result.get("data", {}) if isinstance(result, dict) else {}
        task_id = data.get("task_id") or result.get("task_id")
        if not task_id:
            print("✅ 文档移动操作已完成（无task_id返回）")
            return result
        print(f"⏳ 文档移动进入异步任务，task_id: {task_id}，开始轮询...")
        start_time = time.time()
        while True:
            task_result = self.get_wiki_task_result(task_id)
            status = task_result.get("data", {}).get("status") or task_result.get("status")
            # 常见状态：success/failed/processing，具体以官方为准
            if status in ("success", "succeeded", "done", "finished"):
                print("🎉 文档移动已完成")
                return task_result
            if status in ("failed", "error", "canceled"):
                raise Exception(f"文档移动任务失败: {task_result}")
            if time.time() - start_time > timeout_seconds:
                raise TimeoutError(f"等待文档移动任务超时({timeout_seconds}s): {task_result}")
